node {
    ...
    stage('Preparation') {
        // fetch the code
        git 'http://github.com/johatfie/gateway-server.git'
    ...
    stage('Build image') {
        sh "'${mvnHome}/bin/mvn' -Ddocker.image.prefix=326608040956.dkr.ecr.us-east-2.amazonaws.com/widgets-are-us -Dproject.artifactId=gateway-server -Ddocker.image.version=0.0.1-SNAPSHOT dockerfile:build"
    }
    stage('Push image') {
        docker.withRegistry('https://326608040956.dkr.ecr.us-east-2.amazonaws.com/widgets-are-us', 'ecr:us-east-2:ecr-user') {
            sh "docker push 326608040956.dkr.ecr.us-east-2.amazonaws.com/widgets-are-us/gateway-server:0.0.1-SNAPSHOT"
        }
    }
    stage('Kubernetes deploy') {
        ...
        withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'kubeconfig file', namespace: '', serverUrl: '') {
            sh "sed -ie \"s/THIS_STRING_IS_REPLACED_DURING_BUILD/\$(date)/g\" gateway-server-deployment.yaml"
            sh "/usr/local/bin/kubectl apply -f gateway-server-deployment.yaml"
        }
    }
